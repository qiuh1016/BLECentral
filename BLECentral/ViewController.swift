...
class ViewController: UIViewController,CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    override func viewWillDisappear(animated: Bool) {
        ...
        NSLog("扫描停止")
        terminalTextAppend("scan stopped")
    ...
    */
    ...
    func centralManager(central: CBCentralManager, didDiscoverPeripheral peripheral: CBPeripheral, advertisementData: [String : AnyObject], RSSI: NSNumber) {
        ...
        if self.discoveredPeripheral != peripheral {
            ...
            terminalTextAppend("peripheral connected: \(peripheral)")
    ...
    func centralManager(central: CBCentralManager, didConnectPeripheral peripheral: CBPeripheral) {
        NSLog("外设已连接")
        terminalTextAppend("peripheral connected")
        self.stop()
        NSLog("扫描停止")
        terminalTextAppend("scan stopped")
        self.data.length  = 0
    ...
    func centralManager(central: CBCentralManager, didFailToConnectPeripheral peripheral: CBPeripheral, error: NSError?) {
        ...
        terminalTextAppend("peripheral connect failed, \(peripheral), error: \(error?.localizedDescription)")
        self.cleanup()
    }
    
    func centralManager(central: CBCentralManager, didDisconnectPeripheral peripheral: CBPeripheral, error: NSError?) {
        NSLog("外设已经断开")
        terminalTextAppend("peripheral disconnected")
    ...
    */
    func peripheral(peripheral: CBPeripheral, didDiscoverServices error: NSError?) {
        if error != nil {
            ...
            terminalTextAppend("discover services failed")
            self.cleanup()
            return
        }
    ...
    */
    func peripheral(peripheral: CBPeripheral, didDiscoverCharacteristicsForService service: CBService, error: NSError?) {
        if error != nil {
            ...
            terminalTextAppend("discover characteristics failed")
            self.cleanup()
            return
        }
    ...
    */
    func peripheral(peripheral: CBPeripheral, didUpdateValueForCharacteristic characteristic: CBCharacteristic, error: NSError?) {
        ...
        if error != nil {
            ...
            terminalTextAppend("UpdateValueForCharacteristic failed, error: \(error!.localizedDescription)")
            return
        }
    ...
    func peripheral(peripheral: CBPeripheral, didUpdateNotificationStateForCharacteristic characteristic: CBCharacteristic, error: NSError?) {
        if error != nil {
            ...
            terminalTextAppend("UpdateNotificationStateForCharacteristic failed, error: \(error!.localizedDescription)")
            return
        }
        ...
        if characteristic.isNotifying {// 特征通知已经开始
            ...
            terminalTextAppend("NotificationStateForCharacteristic turned to TRUE, characteristic: \(characteristic)")
        } else {// 特征通知已经停止
            NSLog("特征通知已经停止 %@", characteristic)
            terminalTextAppend("NotificationStateForCharacteristic turned to FALSE, characteristic: \(characteristic)")
            self.centralManager.cancelPeripheralConnection(peripheral)
        }
